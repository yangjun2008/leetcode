...
class Solution {
    ...
    private Node flattenNode(Node node) {
        ...
        while(originNext != null) {
            Node newNext = originNext.next;
            if(originNext.child != null) {
                flattenChild(originNext);
            }
            if(newNext == null) {
                break;
            }
    ...
    private Node flattenChild(Node node) {
        Node originNext = node.next;
        Node childEndNode = flattenNode(node.child);
        node.child.prev = node;
        node.next = node.child;
        childEndNode.next = originNext;
        originNext.prev = childEndNode;
        node.child = null;
        return childEndNode;
    }

    public Node getTreeNode(String input) {
        if(input == null || input.trim().length() == 0) {
            return null;
        }
        String[] nodeVal = input.trim().split(",");
        Node[] tmpNodes = new Node[nodeVal.length];
