...
public class P133CloneGraph {
    ...
    class Solution {
        public Node cloneGraph(Node node) {
            ...
            return cloneGraph(node, valMap);
        }

        private Node cloneGraph(Node node, Map<Integer,Node> valMap) {
            if(valMap.get(node.val) == null) {
                final Node newNode = new Node(node.val);
                newNode.neighbors = new ArrayList<>();
                valMap.put(node.val, newNode);
                node.neighbors.forEach(neighbor -> newNode.neighbors.add(cloneGraph(neighbor, valMap)));
            }
            return valMap.get(node.val);
        }
    }
